...
public partial class Reports_Templates_addSchistomiasis : System.Web.UI.Page
{
    ...
    private void LoadListView(ListView listviewSchisto)
    {
        data = new DataAccess();
        ...
        SqlCommand cmdTxt = new SqlCommand("SELECT a.SchistoEntryNumber,a.SchistoData,a.Male,a.Female," +
            "a.InputDate,a.BarangayID,a.Accomplishment,a.Month,a.Year,b.BarangayName,b.BarangayID,c.Population,c.Target,c.BarangayID" +
            " FROM Schisto a,Barangays b,Population c WHERE SchistoData = @indicatorData AND a.BarangayID = b.BarangayID AND c.BarangayID = a.BarangayID AND a.Year = c.Year", conn);
        cmdTxt.Parameters.Add("@indicatorData", SqlDbType.VarChar).Value = ddlIndicator.Text;

        dr = cmdTxt.ExecuteReader();

        listviewSchisto.DataSource = dr;
        listviewSchisto.DataBind();

        dr.Dispose();
        conn.Close();
    }
    protected void listviewSchisto_ItemInserting(object sender, ListViewInsertEventArgs e)
    {
        data = new DataAccess();
        ...
        data.InsertSchisReport(ddlIndicator.Text.Trim(),
    ...
    protected void listviewSchisto_ItemDeleting(object sender, ListViewDeleteEventArgs e)
    {
        int IndexItemSelected = e.ItemIndex;
        Label entryNumber = (Label)listviewSchisto.Items[IndexItemSelected].FindControl("SchistoEntryNumberLabelID1");
    ...
    private void DeleteItem(string EntryNumber)
    {
        ...
        SqlCommand cmdTxt = new SqlCommand("Delete From Schisto Where SchistoEntryNumber = @entryNum", conn);
