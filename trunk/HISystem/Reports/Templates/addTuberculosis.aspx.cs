...
public partial class Reports_Templates_addTuberculosis : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            LoadListView(listviewTuberculosis);
        }
    }
    ...
    private void LoadListView(ListView listviewTuberculosis)
    {
        data = new DataAccess();
        ...
        SqlCommand cmdTxt = new SqlCommand("SELECT a.TuberculosisEntryNumber,a.TuberculosisData,a.Male,a.Female," +
            "a.InputDate,a.BarangayID,a.Accomplishment,a.Month,a.Year,b.BarangayName,b.BarangayID,c.Population,c.Target,c.BarangayID" +
            " FROM Tuberculosis a,Barangays b,Population c WHERE TuberculosisData = @indicatorData AND a.BarangayID = b.BarangayID AND c.BarangayID = a.BarangayID AND a.Year = c.Year", conn);
        cmdTxt.Parameters.Add("@indicatorData", SqlDbType.VarChar).Value = ddlIndicator.Text;

        dr = cmdTxt.ExecuteReader();

        listviewTuberculosis.DataSource = dr;
        listviewTuberculosis.DataBind();
    ...
    protected void listviewTuberculosis_ItemInserting(object sender, ListViewInsertEventArgs e)
    {
        data = new DataAccess();
        ...
        data.InsertTbReport(ddlIndicator.Text.Trim(),
    ...
    protected void listviewTuberculosis_ItemDeleting(object sender, ListViewDeleteEventArgs e)
    {
        int IndexItemSelected = e.ItemIndex;
        Label entryNumber = (Label)listviewTuberculosis.Items[IndexItemSelected].FindControl("TBEntryNumberLabelID1");
    ...
    private void DeleteItem(string EntryNumber)
    {
        ...
        SqlCommand cmdTxt = new SqlCommand("Delete From Tuberculosis Where TuberculosisEntryNumber = @entryNum", conn);
